...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const parseTreeExt =
    vscode.extensions.getExtension<ParseTreeExtensionApi>("pokey.parse-tree");
  if (!parseTreeExt) {
    vscode.window.showErrorMessage(
      'Required extension "pokey.parse-tree" not found.'
    );
    return;
  }
  ...
  try {
    ...
    if (!loaded) {
      vscode.window.showErrorMessage(
        'Could not load Python parser for "pokey.parse-tree".'
      );
...
async function updateWebviewContent(
  ...
  const openPythonDocs = vscode.workspace.textDocuments.filter(
    (doc) => doc.languageId === "python" && !doc.isClosed
  );
  ...
  for (const document of openPythonDocs) {
    try {
      ...
      vscode.window.showWarningMessage(
        `Could not parse ${document.uri.fsPath}: ${error}`
      );
    }
  }
...
function parsePythonDocument(
  ...
  const definitions = new Map<
    string,
    { node: Node; type: "class" | "function" }
  >(); // Store last seen definition
...
function parseClassDefinition(
      ...
      if (memberNode.type === "function_definition") {
        ...
        if (assignmentNode?.type === "assignment") {
          ...
          if (leftNode?.type === "identifier") {
            ...
            leftNode?.type === "typed_parameter" ||
            leftNode?.type === "typed_identifier"
          ) {
            // Handle 'var: type'
        ...
        else if (assignmentNode?.type === "typed_identifier") {
          // Simplified check
  ...
  memberDefinitions.forEach((memberNode, memberName) => {
    if (memberNode.type === "function_definition") {
      const funcData = parseFunctionDefinition(
        memberNode,
        memberName,
        "",
        false
      ...
      if (assignmentNode?.type === "assignment") {
        const leftNode = assignmentNode.childForFieldName("left");
        if (leftNode?.type === "typed_identifier") {
          // var: type = val
...
function parseFunctionDefinition(
  ...
  const signatureRight = topLevel
    ? `${paramsText} -> ${returnTypeText}`
    : `(${paramsText.slice(1, -1)}) -> ${returnTypeText}`; // Remove outer () for method params
...
function getWebviewHtml(
  ...
  const scriptUri = webview.asWebviewUri(
    vscode.Uri.joinPath(extensionUri, "out", "diagram.js")
  );
